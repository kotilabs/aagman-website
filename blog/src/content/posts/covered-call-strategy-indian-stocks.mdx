...
description: "Learn the covered call strategy for Indian stocks: how it works, when to use it, strike selection, risks, taxes, and how to backtest it on Aagman."
...
The fastest way to know whether covered calls suit your portfolio is to backtest the exact rules on the stocks you already own.

**1. Backtest it on historical data.** Paste this into Aagman:

```
Backtest a covered call strategy on [STOCK TICKER] from 1 January 2023 to 1 January 2026.
...
You will get the actual premium collected each cycle, how often you got assigned, the effect on your stock returns, and the monthly income profile. Tweak anything, strike distance, expiry, underlying, and it re-runs.

**2. Paper trade it before risking real money.** A backtest assumes clean fills at end-of-day prices. Real covered call writing has its own rhythm: when to sell (often the day after expiry, when premiums reset), how to react when the stock approaches your strike mid-month, when to roll versus let assignment happen. Paper trade two or three cycles to build that feel.

**3. Connect your broker and deploy live.** When paper results match the backtest, connect your broker and ask Aagman to deploy a covered call on [STOCK TICKER] with your chosen strike-distance, expiry, and position-size rules. Risk checks run before every order, and you get a fill alert each time a new call is sold. Start small.
...
- [Best strategies for a sideways market](/blog/best-strategies-sideways-market)
