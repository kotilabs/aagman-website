---
title: "Market vs limit orders: the entry decision on Reliance"
description: "Market orders fill quickly but can slip; limit orders control price but may miss the move. Learn the difference on NSE with a Reliance trade example."
...
tags: ["order types", "market order", "limit order", "reliance", "execution"]
featured: false
---
...
The fastest way to settle the market-vs-limit question for your setup is to backtest the entry on real bid-ask data, then move up to desk-tier order types when size matters.

**1. Backtest the entry.** Paste this into Aagman:

```text
Backtest a market-entry strategy on RELIANCE futures on a 15-minute timeframe from 1 Jan 2024 to 31 Dec 2024. Buy at market when the close crosses above the previous day high. Record the LTP at the signal, the best offer, the actual market fill, and the slippage in bps.
```

The report shows average slippage, fill rate, and how many breakouts you would have missed with a limit order.

**2. Paper trade it.** Paper trading catches what a backtest cannot: your broker's latency, the real bid-ask spread at the breakout, and whether you can sit through a limit order that does not fill.

**3. Move up to desk-tier orders when size grows.** For larger size or tighter execution, Aagman can run market-if-touched, limit-if-touched, and VWAP/TWAP slices on your existing broker account. Start with:

```text
Paper trade a market-if-touched order on RELIANCE futures: buy 1 lot if the price touches ₹2,950, using a limit of ₹2,950.50. Stop loss at ₹2,930.
```
...
- [Covered call strategy for Indian stocks](/blog/covered-call-strategy-indian-stocks/) — build the stock leg with a limit order, then rent the upside.
- [Cash secured put: getting paid to buy stocks](/blog/cash-secured-put-getting-paid-to-buy-stocks/) — entry order matters when you want to be assigned at a strike.
- [Bull call spread: directional debit spread](/blog/t1-bull-call-spread-directional/) — limit entry is critical when opening two legs at a target net debit.
- [Short straddle on Nifty for income](/blog/t1-short-straddle-nifty-income/) — market vs limit execution when selling premium at the open.
- [Iron condor on Nifty](/blog/t1-trade-iron-condor-nifty/) — order type choice for multi-leg entry at defined prices.
- [Why options buyers lose money](/blog/why-options-buyers-lose-money/) — how entry slippage adds to the cost edge working against buyers.